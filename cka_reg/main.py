...
def set_logger(hparams):
    logger = TensorBoardLogger(
        hparams.log_save_path, name=hparams.file_name, version=hparams.v_num
    )
...
def add_path_names(hparams):
    ...
    # hparams.log_save_path = os.path.join(os.path.expanduser('~/data2/CKAReg_results'), hparams.save_path)
    hparams.log_save_path = f"{PROJECT_ROOT}/results/{hparams.save_path}"
...
def get_args(*args):
    ...
    parent_parser.add_argument(
        ...
        type=int,
        default=42,
        help="seed for selecting neurons. ",
    )
    parent_parser.add_argument(
        "--save_path",
        metavar="DIR",
    ...
    parent_parser.add_argument(
        "--num_workers", type=int, default=4, help="how many workers"
    )
    parent_parser.add_argument(
        "--num_nodes", type=int, default=1, help="how many nodes"
    )
    ...
    parent_parser.add_argument(
        "-BH",
        "--behanvior_benchmarks",
        dest="behavior_benchmarks",
        nargs="*",
        default=["None"],
    ...
    parent_parser.add_argument(
        ...
        nargs="*",
        default=["All"],
        help="how many of the train neurons to fit to per animal",
    )
    parent_parser.add_argument(
        "-s",
        "--stimuli",
    ...
    parent_parser.add_argument(
        "-ntt",
        "--neural-train-transform",
        dest="neural_train_transform",
        type=int,
        default=1,
    ...
    parent_parser.add_argument(
        "-btt",
        "--behavior-train-transform",
        dest="behavior_train_transform",
        type=int,
        default=1,
        help="if 1, train with input aug on behavioral data; if 0, no input aug",
    )
    parent_parser.add_argument(
        "--translate",
        dest="translate",
    ...
    parent_parser.add_argument(
        ...
        dest="shear",
        type=str,
        default="(0.9375, 1.0625, 0.9375, 1.0625)",
