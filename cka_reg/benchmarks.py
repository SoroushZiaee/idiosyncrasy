...
def brain_wrap_model(identifier, model, layers, image_size):
    ...
    brain_model = ModelCommitment(
        identifier=identifier, activations_model=activations_model, layers=layers
    )
...
def score_model(
    model_identifier, model, benchmark_identifier, layers=[], image_size=224
):
    import os
    ...
    if "cornet" in model_identifier:
        brain_model = cornet_s_brainmodel(
            identifier=model_identifier, model=model, image_size=image_size
        )
    else:
        brain_model = brain_wrap_model(
            identifier=model_identifier, model=model, layers=layers, image_size=224
        )
...
class BehaviorScorer:
    ...
    def get_target(self, metric):
        ...
        if metric == "i1":
            target = h5.File(f"{NEURAL_DATA_PATH}/neural_data/i1_hvm640.mat", "r+")[
                "i1_hvm640"
            ]
        elif metric == "i1n":
            target = h5.File(f"{NEURAL_DATA_PATH}/neural_data/i1n_hvm640.mat", "r+")[
                "i1n_hvm640"
            ]
    ...
    def get_features(self, model_identifier, model, layer, image_size):
        ...
        features = wrapped_model.get_activations(stimuli, layer_names=[layer])[
            layer
        ].squeeze()
    ...
    def compute_i1(self, features):
        features = features.T
        nrImages = features.shape[
            1
        ]  # number of images retrieved from the feature matrix
...
def get_percent_correct_from_proba(prob, labels, class_order):
    ...
    for i in range(nrImages):
        ...
        pc[i, :] = np.divide(
            prob[i, labels[i] == class_order], prob[i, :] + prob[i, loc_target]
        )
...
def list_behavior_benchmarks():
    benchmarks = ["HVM640.All_i1", "HVM640.All_i1n", "HVM640.6_i1", "HVM640.6_i1n"]
